...
const Services = () => {
  ...
  return (
    <section id="services" className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-emerald-50/70 via-white to-blue-50/50">
              ...
              <div className="absolute inset-0 p-4 sm:p-6 flex flex-col justify-center text-white z-10">
                ...
                <div className="brief text-xs sm:text-sm text-white/90 font-normal leading-relaxed max-w-md mx-auto drop-shadow-lg mb-6 sm:mb-8 px-4 text-center">
                  {visibleServiceMobile.brief.split('. ').map((sentence, idx) => (
                    <span key={idx} className="block mb-1.5 last:mb-0">{sentence.trim()}.</span>
                  ))}
                </div>
            ...
            <div className="p-4 sm:p-8 flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                {currentService.features.map((feature, idx) => (
                  <div key={idx} className="flex items-center space-x-2 sm:space-x-3 p-3 sm:p-4 bg-emerald-50/80 backdrop-blur-sm rounded-xl border border-emerald-200/50 hover:bg-emerald-100 hover:shadow-md transition-all duration-200">
                    <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 bg-emerald-500 rounded-full shadow-sm flex-shrink-0" />
                    <span className="font-medium text-gray-800 text-xs sm:text-sm">{feature}</span>
                  </div>
                ))}
              </div>
            </div>
